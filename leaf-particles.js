// leaf-particles.js - Reusable leaf particles for all pages (tidied)
(function () {
  'use strict';
  ...
  class LeafParticles {
    ...
    startContinuousSpawning() {
      // store interval id so we could clear it in future (not strictly necessary now)
      this._intervalId = setInterval(() => {
        if (Math.random() > 0.7) this.spawnRandomLeaf();
      }, 2000);
    }
    ...
    createLeaf(x, y) {
      ...
      // fallback removal
      setTimeout(() => { if (leaf.parentNode) leaf.remove(); }, duration + delay + 300);
    }
    ...
    spawnInteractiveLeaf(x, y) {
      ...
      anim.onfinish = () => leaf.remove();
      setTimeout(() => { if (leaf.parentNode) leaf.remove(); }, duration + 200);
    }
  }
  ...
  if (!window.__LeafParticlesInitialized) {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => { new LeafParticles(); window.__LeafParticlesInitialized = true; });
    } else {
      new LeafParticles();
